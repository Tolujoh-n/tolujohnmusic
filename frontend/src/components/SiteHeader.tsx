    ...
    <header className="fixed inset-x-0 top-0 z-40 bg-gradient-to-b from-slate-950/90 via-slate-950/30 to-transparent backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 transition-all">
        <NavLink to="/" className="group flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <p className="text-xs uppercase tracking-[0.4em] text-white/60">Music Artist</p>
          </div>
        </NavLink>
        ...
        <nav className="hidden items-center gap-8 md:flex">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  ...
                  isActive ? 'text-accent-200 drop-shadow-md' : 'text-white/70 hover:text-white',
                ]
                  .filter(Boolean)
                  .join(' ')
              }
          ...
          <NavLink
            to="/tour"
            className="rounded-full border border-white/30 px-4 py-2 text-xs font-semibold uppercase tracking-[0.4em] text-white transition hover:border-accent-300/80 hover:text-accent-100"
          >
            Tour Dates
          </NavLink>
        </nav>
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-white/5 text-white transition hover:border-white md:hidden"
      ...
      {open && (
        <div className="border-t border-white/10 bg-slate-950/95 backdrop-blur md:hidden">
          <nav className="mx-6 flex flex-col gap-4 py-6 text-sm uppercase tracking-[0.4em] text-white/70">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
